  ...
  return (
    <div className="mx-auto w-[90%] max-w-[1600px] px-4 py-6">
      ...
      {loading ? (
        ...
        <div className="py-16">
          <QuoteScroller quotes={QUOTES} spinner heading="Loading your posts…" classNames={quoteStyles} />
        </div>
      ) : posts.length === 0 ? (
        ...
        <>
          <TableShell classNames={tableShellStyles}>
            ...
            <tbody>
              ...
              // The badge said only THAT a fallback happened; the reason lived
              // in the hover tooltip — invisible on a phone with no pointer,
              // which is where the owner reads this. Lift the distinct reason(s)
              // out so the "why" is on the row itself (dedup: four blocks all
              // "out of credit" is one sentence, not four).
              const fellLabels = Array.from(
                new Set((p.fell_back ?? []).map((f) => attemptLabel(f.reason ?? "unreported"))),
              );
              return (
                <tr
                  key={p.post_id}
                  ...
                  <td className="px-3 py-2.5 align-top">
                    ...
                    {fellLabels.length > 0 && (
                      ...
                      >
                        ...
                        <span>fell back — {fellLabels.join("; ")}</span>
                      </span>
                    )}
